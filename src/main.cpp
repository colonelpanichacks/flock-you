...
// ============================================================================
// RAVEN UUID DETECTION (NimBLE wrappers — delegates to fy_detect.h)
// ============================================================================
...
static bool checkRavenUUID(NimBLEAdvertisedDevice* device, char* out_uuid = nullptr) {
    ...
    for (int i = 0; i < n; i++) {
        bufs[i] = device->getServiceUUID(i).toString();
        strs[i] = bufs[i].c_str();
    }
    return checkRavenUUIDFromStrings(strs, n, out_uuid);
}
...
static const char* estimateRavenFW(NimBLEAdvertisedDevice* device) {
    ...
    return estimateRavenFWFromFlags(has_new_gps, has_old_loc, has_power);
}
